...
- **`CredentialService` (DMN-084)**: the same store, managed by API instead of the CLI — this is how the platform pushes an organization's registry tokens onto a node without ever running `asc auth add` over SSH. `ListCredentials`/`UpsertCredential`/`RemoveCredential` operate on the **system store only** (`/etc/asc/auth.json`) — a user's own `~/.asc/auth.json` stays exclusively under the CLI. `UpsertCredential` carries exactly the same `(kind, pattern, app)` upsert semantics as `GitAuth::add` above — calling it again with the same triple replaces the token, so the platform can safely retry a push. **Token credentials only**: an SSH key is a path to a file that must already exist on this node, which the API has no business creating — `UpsertCredential` always writes a `Method::Token` entry, and an existing SSH-key credential at the same `(kind, pattern, app)` can only be removed or left alone, never overwritten this way. `ListCredentials` returns the same "type, pattern, method label" shape the CLI's `list` prints — never the token itself.
...
DMN-003, DMN-018, DMN-038, DMN-040, DMN-045, DMN-046, DMN-047, DMN-048, DMN-052, DMN-053, DMN-059, DMN-084, REG-001, REG-002, REG-005, BE-002, BE-003 in [ROADMAP.md](../../../asc-platform/ROADMAP.md); GRW-011 in [ROADMAP-GROWTH.md](../../../asc-platform/ROADMAP-GROWTH.md).
