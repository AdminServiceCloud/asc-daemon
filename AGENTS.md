...
4. **Versioning — Semantic Versioning** (`MAJOR.MINOR.PATCH`, [semver.org](https://semver.org)):
   ...
   **The version bump ships with the change**: after landing any new feature or user-visible fix, bump the version in `Cargo.toml` (plus `Cargo.lock` and [version.txt](version.txt)) in the same change — a feature without a version bump is not done. Also update the `Version` badge in [README.md](README.md) and [docs/russian/root.md](docs/russian/root.md) (`img.shields.io/badge/version-X.Y.Z-blue`) — both currently hardcode the version rather than reading `version.txt`. After the change lands on `main`, push the release tag: `git tag vX.Y.Z && git push origin vX.Y.Z` (triggers `release.yml`: cross-builds + GitHub Release).
