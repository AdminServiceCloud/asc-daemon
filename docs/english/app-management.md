...
- **Image version and repull** (DMN-120): **`AppService.GetAppImage`** reports which image a Docker app runs — reference, tag, version (the `org.opencontainers.image.version` label, else a non-`latest` tag), the local manifest digest from `RepoDigests` and the id of the image the container was created from — and, with `check_remote`, the digest the registry serves for the tag right now (`/distribution/{name}/json` through the Engine: a manifest lookup, no layers, bounded at 15 s, same registry credentials as install). The verdict is `RESTART_REQUIRED` when the tag already points at a newer local image than the container runs, else `UP_TO_DATE`/`UPDATE_AVAILABLE` by digest, else `UNKNOWN` (a local build, nothing pulled, or the registry did not answer). **`AppService.RepullApp`** pulls a `latest` (or untagged) registry image afresh and restarts a running app when the image changed; a pinned tag, a digest, a locally built image or a non-Docker app is refused with `FAILED_PRECONDITION`. The restart needs no separate recreate path: the settings-drift check (`pkg::refresh`) now compares the container's image id with the id the reference points at, so any start/restart after a pull — including a manual `docker pull` — recreates the container onto the new image, like `docker compose up`. Capability `app.repull`.
- **`asc app clone <name> [--name <clone-name>]`** (DMN-019): a full copy of an app instance — repository, config and data — under a new id (the same `<id>-N` numbering a repeat `asc install` uses, DMN-033), with a live byte-progress bar over the copy on a terminal. The runtime itself is never copied (a Docker container/systemd unit/process cannot be), only recreated from the copied manifest and settings, exactly like a fresh install. The clone always starts stopped. **`AppService.CloneApp`/`CloneAppStream`** (DMN-113) expose the same operation over gRPC/REST (`POST /v1/apps/{id}/clone`) for the platform's "Clone" button, the streamed sibling reporting the copy's progress as text lines (one per whole percentage point) the same way `InstallAppStream`/`UpgradeAppStream` do. Cloning to a *different* node (NODE-045) reuses this locally, on the target, as the "install" phase of a cross-node move — see [🧬 app-cloning](../../../asc-platform/docs/features/app-cloning.md). That move needs to re-derive an install spec from the app's own recorded origin the way `asc app upgrade` already does internally (`meta.source`/`meta.package`/`meta.repo_path`/`meta.branch`), so `App` (`ListApps`/`GetApp`) now also carries `package`, `repo_path` and `branch` alongside the existing `source`/`version`/`commit` — all optional, all absent on an app installed before DMN-113.
...
- **`asc ports [<name>]`** / **`asc app ports [<name>]`** (DMN-049): the published ports of an app with their transport (`27015/tcp`, `27015/udp`, or `27015/tcp+udp` when both share the port), resolved from the app's `type: ports` settings — so a **stopped** app still reports the ports it will bind on the next start. A package that fixes its container side (`container:`, DMN-052) reads as `8080->3000/tcp`: the host port first, then the container port behind it. Without a name: a table of every visible app and its ports (root sees all users' apps). **`AppService.ListAppPorts`** (DMN-079) is the same list over gRPC, for the platform: docker apps report their ports, every other kind returns an empty list.
...
- **An image or volume an installed app declares is protected — running or
  ...
  declares), never from labels or naming conventions. `asc docker images`/
  `volumes` mark a protected row and say which app is holding it.
- **Removal is one item at a time, never the Engine's own bulk `/images/prune`
  or `/volumes/prune`.** A container ASC did not create carries no ownership
  label, so a filtered bulk prune would either miss nothing (no filter) and
  risk removing something in active use through a race, or filter on a label
  this daemon only started adding with DMN-105 and therefore leaves every
  pre-existing container's image unprotected. Per-item removal makes the
  protected-set check exact and makes `--dry-run` report precisely what would
  happen, not an estimate.
- **Networks are inventory-only — there is no `asc docker prune networks`.**
  ASC never creates or deletes networks of its own, and the Engine's own
  network prune removes any network with no *running* container attached,
  including a stopped Compose stack's network — silently breaking its next
  `up`. Showing networks without a delete path avoids that trap entirely.
- **`asc docker df` is not cheap and is never polled.** Like `docker system
  df`, answering it makes the Engine walk every image layer and volume; call
  it when opening a cleanup view, not on a timer.
- **Container labels** (`asc.managed`, `asc.app.id`, `asc.app.uuid`) are
  written on every container ASC creates from DMN-105 onward, for a future
  caller that wants to recognize an ASC-managed container without matching by
  name. They are informational only: the settings-drift check
  (`container_applied`) never reads labels back, so adding them cannot trigger
  an unwanted recreate of an already-running container.
...
DMN-002, DMN-004, DMN-019, DMN-044, DMN-051, DMN-053, DMN-102, DMN-104, DMN-105, DMN-112, DMN-113, FE-005, FE-108, NODE-039, BE-040, FE-110, NODE-044, BE-008, FE-024 in [ROADMAP.md](../../../asc-platform/ROADMAP.md).
