...
Встроенный в демон MCP-сервер (Model Context Protocol) — набор инструментов, через которые AI управляет сервером. Используется AI-ассистентом платформы ([🤖 ai-assistant](../../asc-platform/docs/features/ai-assistant.md)), а также любым MCP-клиентом (Claude Desktop, Claude Code) при локальной работе — сильный ход для open source аудитории.
...
- **Авторизация**: набор доступных инструментов фильтруется правами пользователя, от имени которого работает AI ([🔐 access-control](../../asc-platform/docs/features/access-control.md)).
- **Аудит**: каждый вызов инструмента — в лог демона и в аудит платформы.

## 🔗 Связанные задачи

DMN-013, AI-001, AI-002, AI-003 в [ROADMAP.md](../../asc-platform/ROADMAP.md).
