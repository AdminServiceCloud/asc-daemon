...
- **Разделение путей по пользователям**: `/asc/apps/` (вместе с `/etc/asc/config.toml` и `/var/lib/asc`) — дерево **root**-установки: системный демон и `sudo asc`. Запуск `asc` от обычного пользователя работает со своим приватным деревом в `~/.asc/`: `~/.asc/apps`, `~/.asc/data`, `~/.asc/config.toml` — так пользователь редактирует настройки своих приложений и конфиг без sudo. Root-секция `[policy]` по-прежнему читается из системного конфига и не переопределяется на уровне пользователя.
