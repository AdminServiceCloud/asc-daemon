...
- **`CredentialService` (DMN-084)**: тот же стор, только под управлением по API вместо CLI — так платформа выкладывает токены реестров организации на ноду, не запуская `asc auth add` по SSH. `ListCredentials`/`UpsertCredential`/`RemoveCredential` работают **только с системным стором** (`/etc/asc/auth.json`) — собственный `~/.asc/auth.json` пользователя остаётся исключительно под CLI. `UpsertCredential` несёт ровно ту же семантику апсерта по `(kind, pattern, app)`, что и `GitAuth::add` выше — повторный вызов с той же тройкой заменяет токен, поэтому платформа может безопасно повторить пуш при сбое. **Только токен-учётные данные**: SSH-ключ — это путь к файлу, который должен уже существовать на ноде, а создавать его через API незачем; `UpsertCredential` всегда пишет запись `Method::Token`, а существующую SSH-ключевую запись на той же тройке `(kind, pattern, app)` можно только удалить или оставить как есть, но не перезаписать этим методом. `ListCredentials` отдаёт тот же вид «тип, шаблон, метка метода», что и CLI-шный `list`, — никогда сам токен.
...
DMN-003, DMN-018, DMN-038, DMN-040, DMN-045, DMN-046, DMN-047, DMN-048, DMN-052, DMN-053, DMN-059, DMN-084, REG-001, REG-002, REG-005, BE-002, BE-003 в [ROADMAP.md](../../../asc-platform/ROADMAP.md); GRW-011 в [ROADMAP-GROWTH.md](../../../asc-platform/ROADMAP-GROWTH.md).
