...
- **Current run only (DMN-116)**: every runtime appends each run to one shared log (the container's Engine log, the journal, `app.log`), so a console session starts its backlog at the **start of the application's current run** — after a start or restart the console shows only what this run printed, never the output of earlier runs. Docker `logs`: `since` = the container's `State.StartedAt`, cut exactly on each line's nanosecond timestamp. Docker `attach`: the live attach is opened **without** the Engine's `logs` replay (that replay is every run the container ever had); the backlog — the last 1000 messages of the current run — is read from the log afterwards, bounded by `StartedAt` and the moment the attach went live, so the two meet with no gap and no duplicates. systemd: `journalctl _SYSTEMD_INVOCATION_ID=<the unit's current InvocationID>`. process: the driver records the `app.log` offset on every start (`app.log.run-offset`) and the console follows from it (`tail -c +N -F`, the last `tail` lines of the run). compose: `docker compose logs --since <the earliest StartedAt of the project's running containers>`. Where the run start cannot be determined (never started, a process started by an older daemon) the plain `tail` applies. The browser terminal is fully reset for every new session, so a restart replaces the old output instead of stacking under it.
...
- **Permissions**: `apps.console` controls access to the application console ([🔐 access-control](../../../asc-platform/docs/features/access-control.md)).

## 🔗 Related tasks

DMN-007, DMN-082, DMN-088, DMN-116, FE-006, FE-061 in [ROADMAP.md](../../../asc-platform/ROADMAP.md).
