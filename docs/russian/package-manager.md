...
8. **Прогресс-бары**: в терминале клонирование репозитория (`git clone --progress`) и скачивание Docker-образа отображаются живыми полосами прогресса (в стиле `docker pull`/`docker-compose pull` — по одной полосе на слой образа) — включены по умолчанию, независимо от `asc config debug`. При неинтерактивном вызове (перенаправленный вывод, API демона) полос нет — те же события всегда доступны как debug-логи (`asc config debug on`). **Для API-вызывающих, которым нужен живой прогресс** (DMN-090), есть `AppService.InstallAppStream` — та же установка, что и unary `InstallApp`, но как server-streaming последовательность `InstallAppEvent`: событие `line` на каждый шаг (тот же текст, из которого в терминале рисуются полосы, а не сама перерисовка полосы) по мере выполнения, и одно финальное событие `result` с тем же payload, что вернул бы `InstallApp` — либо обрыв стрима с ошибкой. Отключившийся вызывающий не отменяет установку — она продолжается на демоне независимо от того, смотрит ли кто-то ещё, как и у `InstallApp` при обрыве запроса.
