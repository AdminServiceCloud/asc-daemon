...
- Модуль `src/daemon/monitor/`: `system.rs` — парсеры procfs (чистые функции над `&str`, покрыты unit-тестами) и снятие снапшота; `mod.rs` — `Monitor`: фоновый сэмплер в демоне, интервал и глубина истории задаются в `config.toml` (`[monitor] interval_ms = 100`, `history_samples = 300` — 30 с истории в памяти при 100 мс). Старый ключ `interval_secs` из конфигов, написанных до DMN-072, по-прежнему разбирается (чтобы такие файлы загружались), но интервал больше не задаёт и обратно не пишется — при следующем сохранении он исчезает из `config.toml` (DMN-075). Именно из-за него обновлённый демон мог продолжать отдавать один сэмпл в 10 с: в каждой установке старше DMN-072 лежит `interval_secs = 10`.
- **Живой стрим (DMN-072)**: у `Monitor` — `tokio::sync::broadcast`-канал; каждый `push()` рассылает сэмпл подписчикам (ёмкость 16 — отставший получатель просто теряет старые кадры через `Lagged`, не блокируя сэмплер). `MonitorService.StreamSystemMetrics` подписывается на канал вместо периодического опроса `GetSystemMetrics` — панель платформы получает кадры настолько часто, насколько их снимает демон. Подписка берётся до чтения буфера, а уже снятый сэмпл уходит первым кадром (DMN-075): подключившаяся панель рисует цифры сразу, а не через целый интервал сэмплирования.
- **GPU при 100 мс**: `nvidia-smi` — это спавн процесса, 10 раз в секунду он не переживёт. GPU опрашивается на собственном таймере не чаще раза в секунду (`GPU_POLL_MIN_INTERVAL_MS`), а между опросами в сэмпл подмешивается последнее известное значение.
- **Disk I/O**: чтение/запись по блочным устройствам из `/proc/diskstats` (сектора × 512), скорости — дельтой между сэмплами. Партиции (`sda1` при наличии `sda`, `nvme0n1p1` при наличии `nvme0n1`) и `loop`/`ram`/`zram`-устройства отфильтрованы — остаются только целые диски.
- **Сетевые интерфейсы (DMN-074)**: `MonitorService.ListNetworkInterfaces` + REST `GET /v1/network/interfaces` — отдельно от метрик, потому что нужен полный инвентарь, включая loopback (метрики его сознательно отбрасывают): имя, MAC, MTU, состояние и все IPv4/IPv6-адреса каждого интерфейса.
