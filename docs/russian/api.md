...
- Маршрутизация CLI: команды жизненного цикла (`ls`/`status`/`install`/`app start|stop|restart|logs|remove|info`), а также `app attach` и `app settings` (DMN-043) идут через сокет, когда он существует. Нет файла сокета — CLI работает in-process, как раньше (DMN-041: root по системным путям, пользователь в своём дереве `~/.asc`). Сокет есть, но не отвечает — ошибка для обычного пользователя и in-process-fallback с предупреждением для root (восстановление не должно зависеть от здоровья демона).
...
DMN-005, DMN-007, DMN-042, DMN-043 в [ROADMAP.md](../../../asc-platform/ROADMAP.md).
