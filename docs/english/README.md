...
- **Debug logging**: `asc config debug on|off` toggles `[log] level` between `debug` and `info` in `config.toml` (`RUST_LOG` still overrides it); every command initializes tracing, not just `asc serve`, so e.g. a standalone `asc install` streams Docker image-pull progress to stderr — useful when a long install looks stuck with no output. **The setting is per config file, and the daemon has its own**: with the service running, `asc install` is executed *by the daemon* (DMN-042), so its pull and image-build progress goes to the daemon's log at the level `/etc/asc/config.toml` had when the service started — `sudo asc config debug on && sudo asc service restart`, then `journalctl -u asc -f`. Build steps (DMN-050) are logged at info level, so they show up there without turning debug on at all; `RUST_LOG=asc_daemon=trace` additionally dumps every raw frame of the Engine's build stream.
