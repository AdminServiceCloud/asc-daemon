...
- Подстановка выполняется демоном при установке/обновлении из env-дефолтов пакета (`env:` в `asc.yaml`, включая общие переменные стека). Неразрешённая переменная — или переменная без дефолта — ошибка установки с указанием имени переменной.
...
- Подстановка из *итогового* окружения приложения (значения настроек, env-уровни организации/ноды/приложения — [🌱 environments](../../../asc-platform/docs/features/environments.md)) и предпросмотр команды в UI появятся вместе с пробросом настроек в env (DMN-017).
...
3. **Пользовательское название** (DMN-024): в терминале `asc install` спрашивает название приложения — Enter оставляет значение по умолчанию (название из манифеста пакета), любой другой ввод становится названием приложения. Неинтерактивно — флаг `--name "My Server"`. Название хранится в `meta.json` (`custom_name`), должно быть уникально среди приложений пользователя, переживает обновления, и все команды принимают его наравне с id. Пользовательское название применимо к одному приложению — не к целому стеку.
4. Дальше демон работает с локальной копией: читает `asc.yaml`/`asc.settings.yaml`, собирает/запускает по типу приложения.
5. Для Docker-приложений контейнер создаётся через Engine API; отсутствующий на хосте образ **скачивается автоматически** из его реестра (`runtime.image`; имя без тега означает `latest`) — и при установке, и при обновлении.
