...
- `asc stats` — потребление CPU и памяти по приложениям (аналог `docker stats`); root видит приложения всех пользователей, сгруппированные по владельцам.
...
- **CLI `asc stats`**: таблица ID / KIND / CPU % / MEM по запущенным приложениям; сортировка `--sort cpu|mem` (по умолчанию cpu); у root — группировка по владельцам, как в `asc app list`. Остановленные приложения показываются с прочерками.
