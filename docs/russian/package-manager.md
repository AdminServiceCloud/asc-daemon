...
> ℹ️ В манифесте **нет секции `env:`**: все environment-переменные приложения объявляются в `asc.settings.yaml` — настройкой с ключом `env:` (см. ниже). Один источник истины: что пользователь может настроить — то приложение и получает.

**Тома** (docker-приложения) бывают трёх видов:

- `/путь/в/контейнере` — приватные данные приложения: в этот путь контейнера монтируется **папка data** приложения (`/asc/apps/<id>/data`). Папка создаётся world-writable (0777): образы работают от произвольных непривилегированных пользователей, а bind-монтирование сохраняет владельца хоста; каталог приложения выше остаётся закрытым. Позже это ужесточится маппингом uid на приложение;
- `/путь/в/контейнере:папка` — то же, но вместо `data` используется папка хоста, указанная после двоеточия (`/asc/apps/<id>/<папка>`) — для пакетов с несколькими приватными томами. Имя — простое имя папки; `repository`, `config` и `meta.json` зарезервированы;
...
- **Проброс в env**: каждая настройка с `env: VAR_NAME` попадает в окружение приложения (секреты тоже — для этого их `env:` и нужен). Для Docker-приложений переменные записываются в env контейнера при создании. Настройки — **единственный** источник env: в `asc.yaml` секции `env:` нет.
- **Применение изменений**: env контейнера фиксируется при создании, поэтому при следующем `asc app start` / `asc app restart` демон сравнивает желаемый env с фактическим env контейнера и **пересоздаёт контейнер** при расхождении (или если контейнер пропал). Данные приложения живут в томах и переживают пересоздание. Если желаемый env вычислить не удалось (например, источник реестра удалён) — приложение всё равно стартует как есть, с предупреждением в логе: доступность важнее.
...
3. **Согласие с лицензией** (DMN-028): если клонированный репозиторий содержит лицензию (`LICENSE.md` / `LICENSE` / `LICENSE.txt` в корне), CLI показывает, откуда ставится пакет (источник реестра + репозиторий), выводит текст лицензии и спрашивает согласие — отказ прерывает установку, не оставляя следов. Неинтерактивный ввод принимает лицензию автоматически с печатным уведомлением; вызовы через API получают структурированную ошибку с источником, репозиторием и текстом лицензии (UI платформы отрисует свой диалог согласия). Для стека вопрос задаётся один раз на репозиторий. Репозитории без файла лицензии ставятся без вопроса.
4. **Пользовательское название** (DMN-024): в терминале `asc install` спрашивает название приложения — Enter оставляет значение по умолчанию (название из манифеста пакета), любой другой ввод становится названием приложения. Неинтерактивно — флаг `--name "My Server"`. Название хранится в `meta.json` (`custom_name`), должно быть уникально среди приложений пользователя, переживает обновления, и все команды принимают его наравне с id. Пользовательское название применимо к одному приложению — не к целому стеку.
