...
- **`type: ports`** — публикуемые порты контейнера (порт хоста == порту контейнера). Настройка с `env:` держит приложение и Docker синхронными: сервер слушает ровно там, куда проброшен порт.
