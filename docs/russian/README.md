...
- **Отладочные логи**: `asc config debug on|off` переключает `[log] level` между `debug` и `info` в `config.toml` (`RUST_LOG` по-прежнему в приоритете); трассировка инициализируется в любой команде, а не только в `asc serve` — например, автономный `asc install` печатает в stderr прогресс скачивания Docker-образа, что полезно, когда долгая установка выглядит зависшей. **Настройка живёт в конкретном файле конфига, а у демона он свой**: при запущенном сервисе `asc install` выполняет *демон* (DMN-042), поэтому прогресс скачивания и сборки образа уходит в лог демона с тем уровнем, который был в `/etc/asc/config.toml` на момент старта сервиса — `sudo asc config debug on && sudo asc service restart`, дальше `journalctl -u asc -f`. Шаги сборки (DMN-050) пишутся на уровне info и видны там даже без включённого debug; `RUST_LOG=asc_daemon=trace` дополнительно выгружает каждый сырой кадр build-стрима Engine.
