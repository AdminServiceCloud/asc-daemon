...
4. Для Docker-приложений контейнер создаётся через Engine API; отсутствующий на хосте образ **скачивается автоматически** из его реестра (`runtime.image`; имя без тега означает `latest`) — и при установке, и при обновлении.
